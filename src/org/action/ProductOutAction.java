...
//        Product product = (Product) Hsession2.load(Product.class, prodById.getId());
	...
	public String getAllOnShippingProduct() {
	  ...
	  public String addNewProductSold() { boolean valid = false; SessionFactory
	  sessionFactory = new
	  Configuration().configure("hibernate.cfg.xml").buildSessionFactory(); Session
	  Hsession=sessionFactory.openSession(); Transaction ts =
	  Hsession.beginTransaction();
	  
	  SessionFactory sessionFactory2 = new
	  Configuration().configure("hibernate.cfg.xml").buildSessionFactory(); Session
	  Hsession2=sessionFactory2.openSession(); Transaction ts2 =
	  Hsession2.beginTransaction();
	  
	  ProductOut productOut = new ProductOut(); Product prod = new Product();
	  
	  try{ productDao = new ProductDaoImp(); //Product
	  product=productDao.getOneProduct(Integer.parseInt(productInBean.getProd_id())
	  ); //Product product = (Product) Hsession2.get(Product.class,
	  Integer.parseInt(productInBean.getProd_id())); Product
	  productInTheDatabase=productDao.getOneProduct(productOutBean.getProd_id());
	  ...
	  if(productInTheDatabase.getIn_stock() < productOutBean.getQuantity() ||
	  productInTheDatabase.getIn_stock() == 0) { return ERROR; } else {
	  prod.setIn_stock(productInTheDatabase.getIn_stock()-productOutBean.
	  getQuantity()); }
	  
	  prod.setPending_stock(productInTheDatabase.getPending_stock());
	  prod.setDescription(productInTheDatabase.getDescription());
	  
	  Hsession2.update(prod); ts2.commit();
	  ...
	  productOut.setStatus(productOutBean.getStatus()); productOut.setDate(new
	  Date(System.currentTimeMillis()));
	  
	  Hsession.save(productOut); ts.commit();
	  
	  valid = true; }catch(Exception e){ e.printStackTrace();
	  
	  } if(valid){ return getAllProduct(); } else{ return ERROR; }
